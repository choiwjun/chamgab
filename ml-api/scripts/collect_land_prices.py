...
def main() -> None:
    ...
    parser.add_argument(
        "--year",
        type=int,
        default=_env_int("LAND_PRICE_YEAR", datetime.now().year - 1, min_value=2000),
    )
    ...
    parser.add_argument(
        "--min-success-count",
        type=int,
        default=_env_int("LAND_PRICES_MIN_SUCCESS_COUNT", 0, min_value=0),
    )
    parser.add_argument(
        "--min-success-rate-pct",
        type=float,
        default=_env_float("LAND_PRICES_MIN_SUCCESS_RATE_PCT", 0.0, min_value=0.0),
    )
    parser.add_argument(
        "--soft-fail",
        action="store_true",
    ...
    if total > 0 and success == 0:
        LOG.warning(
            "collect_land_prices completed with zero success rows (year=%s, missing=%d, failed=%d)",
            args.year,
            missing,
            failed,
        )
    ...
    if hard_fail:
        LOG.error(
            "collect_land_prices hard-fail: success=%d total=%d success_rate=%.2f%% "
