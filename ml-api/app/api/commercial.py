...
def _try_get_supabase():
    """Supabase 클라이언트를 안전하게 가져옴. 실패 시 None 반환."""
    try:
...
# Supabase 미연결 시 사용할 샘플 데이터
_SAMPLE_FOOT_TRAFFIC = {
    "time_06_11": 3500, "time_11_14": 8000, "time_14_17": 4500,
    "time_17_21": 9500, "time_21_24": 2000,
    "age_10s": 1000, "age_20s": 8000, "age_30s": 12000,
    "age_40s": 6000, "age_50s": 2000, "age_60s": 1000,
    "weekday_avg": 28000, "weekend_avg": 18000, "weekend_ratio": 39.1,
}

_SAMPLE_DISTRICT_CHAR = {
    "district_type": "오피스상권",
    "primary_age_group": "30-40대",
}
...
async def get_peak_hours(code: str):
    ...
    # Supabase 또는 샘플 데이터
    data = _SAMPLE_FOOT_TRAFFIC
    client = _try_get_supabase()
    if client:
        try:
            result = client.table('foot_traffic_statistics') \
                .select('*').eq('commercial_district_code', code).execute()
            if result.data:
                data = result.data[0]
        except Exception:
            pass
...
async def get_demographics(code: str):
    ...
    for age, count in ages.items():
        ...
        demographics[age] = AgeGroupScore(count=count, percentage=round(percentage, 1), score=score)

    primary_target = max(demographics.items(), key=lambda x: x[1].percentage)[0] if total > 0 else "20s"
    ...
    industry_mapping = {
        ...
        "20s": [IndustryMatch(code="Q12", name="커피전문점", match_score=95), IndustryMatch(code="Q06", name="치킨전문점", match_score=88)],
        "30s": [IndustryMatch(code="Q01", name="한식음식점", match_score=85), IndustryMatch(code="Q12", name="커피전문점", match_score=82)],
        "40s": [IndustryMatch(code="Q01", name="한식음식점", match_score=88), IndustryMatch(code="Q03", name="중식음식점", match_score=75)],
        "50s": [IndustryMatch(code="Q01", name="한식음식점", match_score=90), IndustryMatch(code="Q11", name="일반음식점", match_score=80)],
        "60s": [IndustryMatch(code="Q01", name="한식음식점", match_score=92), IndustryMatch(code="Q11", name="일반음식점", match_score=85)],
    }
...
async def get_district_profile(code: str):
    ...
    char_data = _SAMPLE_DISTRICT_CHAR
    client = _try_get_supabase()
    if client:
        try:
    ...
    customer_map = {
        "10-20대": "학생 및 청년층", "20-30대": "MZ세대 직장인",
        "30-40대": "가족 단위 고객", "40-50대": "중장년층", "50대 이상": "시니어층",
    }
    lifestyle_map = {
    ...
    response = DistrictProfileResponse(
        ...
        best_industries=best_industries_map.get(district_type, ["커피전문점", "한식음식점", "편의점"]),
    )
    cache.set(cache_key, response)
    return response
...
async def get_competition_analysis(code: str):
    ...
    alternatives = [
        AlternativeDistrict(code="1168053600", name="본오동", distance=0.8, store_count=42, success_rate=72.0, reason="낮은 경쟁도, 높은 성공률"),
        AlternativeDistrict(code="1168053700", name="간석동", distance=1.2, store_count=68, success_rate=65.0, reason="적절한 경쟁 환경"),
    ]
...
async def get_growth_potential(code: str):
    ...
    sales_data = _SAMPLE_SALES
    business_data = _SAMPLE_BUSINESS
    client = _try_get_supabase()
    if client:
        try:
            sr = client.table('sales_statistics').select('*').eq('commercial_district_code', code).execute()
            br = client.table('business_statistics').select('*').eq('commercial_district_code', code).execute()
            if sr.data:
                sales_data = sr.data[0]
            if br.data:
                business_data = br.data[0]
        except Exception:
            pass
...
class AnalysisSummary(BaseModel):
    ...
    peak_time: str
    weekend_ratio: float  # 0.0 ~ 1.0 비율
...
async def recommend_industry(code: str):
    ...
    **분석 요소:**
    ...
    # Supabase 또는 샘플 데이터
    foot_traffic_data = _SAMPLE_FOOT_TRAFFIC
    client = _try_get_supabase()
    if client:
        try:
            ftr = client.table("foot_traffic_statistics").select("*").eq("commercial_district_code", code).execute()
            if ftr.data:
                foot_traffic_data = ftr.data[0]
        except Exception:
            pass
    ...
    age_percentages = {k: (v / total_age) * 100 for k, v in ages.items()} if total_age > 0 else {k: 0 for k in ages}
    ...
    if primary_age == "20s" and primary_time in ["evening", "night"]:
        recommendations_list.extend([
            IndustryRecommendation(industry_code="I56220", industry_name="주점 및 비알콜음료점업", match_score=92, expected_monthly_sales=2500000, breakeven_months=8, reasons=["20대 고객 비중 높음 (MZ세대 선호)", "저녁~야간 유동인구 집중", "소셜 다이닝 트렌드 부합"]),
            IndustryRecommendation(industry_code="Q12", industry_name="커피전문점", match_score=88, expected_monthly_sales=3000000, breakeven_months=10, reasons=["20대 주 고객층", "카페 문화 확산", "테이크아웃 수요 증가"]),
            IndustryRecommendation(industry_code="Q06", industry_name="치킨전문점", match_score=85, expected_monthly_sales=4000000, breakeven_months=12, reasons=["젊은 층 선호 메뉴", "배달 수요 높음", "야간 영업 유리"]),
        ])
    elif primary_age in ["30s", "40s"] and primary_time == "lunch":
        recommendations_list.extend([
            IndustryRecommendation(industry_code="Q01", industry_name="한식음식점", match_score=90, expected_monthly_sales=3500000, breakeven_months=9, reasons=["직장인 주 고객층", "점심 시간대 집중", "안정적인 수요"]),
            IndustryRecommendation(industry_code="Q10", industry_name="분식전문점", match_score=83, expected_monthly_sales=2800000, breakeven_months=7, reasons=["빠른 식사 선호", "가성비 메뉴", "회전율 높음"]),
            IndustryRecommendation(industry_code="Q09", industry_name="패스트푸드점", match_score=80, expected_monthly_sales=5000000, breakeven_months=15, reasons=["빠른 서빙", "브랜드 인지도", "점심 시간 집중"]),
        ])
    elif primary_age in ["50s", "60s"]:
        recommendations_list.extend([
            IndustryRecommendation(industry_code="Q01", industry_name="한식음식점", match_score=88, expected_monthly_sales=3200000, breakeven_months=10, reasons=["중장년층 선호 메뉴", "지역 커뮤니티 중심", "안정적 고객층"]),
            IndustryRecommendation(industry_code="Q14", industry_name="제과점", match_score=82, expected_monthly_sales=2700000, breakeven_months=11, reasons=["아침 식사 대용", "선물 수요", "다양한 연령층"]),
            IndustryRecommendation(industry_code="N99303", industry_name="편의점", match_score=78, expected_monthly_sales=4500000, breakeven_months=14, reasons=["생활 밀착형", "24시간 운영 가능", "안정적 매출"]),
        ])

    if weekend_ratio > 60:
        recommendations_list.append(IndustryRecommendation(industry_code="Q05", industry_name="일식 음식점업", match_score=86, expected_monthly_sales=3800000, breakeven_months=11, reasons=["주말 외식 선호", "가족 단위 고객", "특별한 날 선택"]))

    if not recommendations_list:
        recommendations_list = [
            IndustryRecommendation(industry_code="Q12", industry_name="커피전문점", match_score=75, expected_monthly_sales=2800000, breakeven_months=12, reasons=["범용적 업종", "진입 장벽 낮음", "안정적 수요"]),
            IndustryRecommendation(industry_code="N99303", industry_name="편의점", match_score=72, expected_monthly_sales=4200000, breakeven_months=15, reasons=["생활 필수 업종", "24시간 운영", "다양한 상품군"]),
            IndustryRecommendation(industry_code="Q01", industry_name="한식음식점", match_score=70, expected_monthly_sales=3000000, breakeven_months=10, reasons=["보편적 선호", "안정적 고객층", "다양한 메뉴"]),
        ]
