...
export function SuccessProbabilityCard({
  ...
  const { success_probability, confidence, factors, recommendation, quality_flags } = result

  const getProbabilityBand = (probability: number) => {
    if (probability >= 75) {
      return {
        label: '우수',
        description: '핵심 지표가 안정적입니다. 우선 검토 후보로 볼 수 있습니다.',
        title: '진입 우선 검토 구간',
        circleClassName: 'text-emerald-700 bg-emerald-50 ring-4 ring-emerald-100',
        progressClassName: 'bg-emerald-500',
        badgeClassName: 'border border-emerald-200 bg-emerald-50 text-emerald-700',
        recommendationClassName: 'border border-emerald-200 bg-emerald-50 text-emerald-900',
        positive: true,
      }
    }
    if (probability >= 60) {
      return {
        label: '유망',
        description: '진입 가능성은 충분하지만 비용·경쟁 강도 점검이 필요합니다.',
        title: '조건부 진입 구간',
        ...
        recommendationClassName: 'border border-blue-200 bg-blue-50 text-blue-900',
        positive: true,
      }
    }
    if (probability >= 35) {
      return {
        ...
        recommendationClassName: 'border border-amber-200 bg-amber-50 text-amber-900',
        positive: false,
      }
    }
    return {
      ...
      recommendationClassName: 'border border-rose-200 bg-rose-50 text-rose-900',
      positive: false,
    }
  }
    ...
    <div className="rounded-xl border border-gray-200 bg-white p-8">
      ...
      <div className="mb-8 text-center">
        <div className="mb-4">
          ...
          >
            <div>
              <div className="text-4xl font-bold">{success_probability.toFixed(1)}%</div>
        ...
        <p className="mb-4 text-sm text-gray-600">{probabilityBand.description}</p>

        <div className="flex items-center justify-center gap-2 text-sm text-gray-600">
          <span>분석 신뢰도</span>
          <span className="font-semibold text-gray-900">{confidence.toFixed(1)}%</span>
          <span
      ...
      <div className={`mb-8 rounded-xl p-4 ${probabilityBand.recommendationClassName}`}>
              ...
              <div key={index} className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-sm font-semibold ${factorStyle.textClassName}`}>
                      {factorStyle.prefix}
                      {impact.toFixed(1)}
                    </span>
                  </div>
