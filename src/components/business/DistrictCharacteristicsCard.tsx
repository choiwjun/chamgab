...
export function DistrictCharacteristicsCard({
  ...
  const getDistrictTypeIcon = () => {
    if (district_type.includes('대학')) return <GraduationCap className="w-5 h-5" />
    if (district_type.includes('오피스')) return <Briefcase className="w-5 h-5" />
    if (district_type.includes('주거')) return <Home className="w-5 h-5" />
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8 space-y-8">
      {/* 헤더 */}
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-4">상권 특성 분석</h2>
        <p className="text-gray-600">{district_name}의 상세한 특성을 분석했습니다.</p>
      </div>
        ...
        <div className="p-6 bg-gradient-to-br from-blue-50 to-white rounded-xl border border-blue-100">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-blue-600">{district_type}</p>
            </div>
          </div>
        ...
        <div className="p-6 bg-gradient-to-br from-purple-50 to-white rounded-xl border border-purple-100">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-purple-600">{primary_age_group}</p>
            </div>
          </div>
          <p className="text-sm text-gray-600">전체의 {primary_age_ratio.toFixed(1)}%를 차지</p>
        </div>
      </div>

      {/* 인구 구성 */}
      <div>
        <h3 className="font-semibold text-gray-900 mb-4">인구 구성</h3>
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            <Briefcase className="w-6 h-6 text-gray-600 mx-auto mb-2" />
            <div className="text-2xl font-bold text-gray-900">{office_worker_ratio.toFixed(0)}%</div>
            <div className="text-sm text-gray-600">직장인구</div>
          </div>
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            <Home className="w-6 h-6 text-gray-600 mx-auto mb-2" />
            <div className="text-2xl font-bold text-gray-900">{resident_ratio.toFixed(0)}%</div>
            <div className="text-sm text-gray-600">주거인구</div>
          </div>
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            <GraduationCap className="w-6 h-6 text-gray-600 mx-auto mb-2" />
            <div className="text-2xl font-bold text-gray-900">{student_ratio.toFixed(0)}%</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="p-6 bg-gradient-to-br from-green-50 to-white rounded-xl border border-green-100">
          ...
          <p className="text-sm text-gray-600">추천 영업 시간: {recommended_business_hours}</p>
        </div>

        <div className="p-6 bg-gradient-to-br from-yellow-50 to-white rounded-xl border border-yellow-100">
          <div className="flex items-center gap-3 mb-4">
          ...
          <p className="text-sm text-gray-600">소비 수준: {consumption_level}</p>
        </div>
      </div>

      {/* 시간대별 유동인구 */}
      <div>
        <h3 className="font-semibold text-gray-900 mb-4">시간대별 유동인구</h3>
        <div className="h-64">
      ...
      {/* 연령대별 분포 */}
      <div>
        <h3 className="font-semibold text-gray-900 mb-4">연령대별 분포</h3>
      ...
      <div className="p-6 bg-gray-50 rounded-xl">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="text-sm text-gray-600 mb-1">주중/주말 패턴</p>
            <p className="font-semibold text-gray-900">
              {weekday_dominant ? '주중 우세' : '주말 우세'}
            </p>
          </div>
          <div>
            <p className="text-sm text-gray-600 mb-1">주말 매출 비중</p>
            <p className="font-semibold text-gray-900">{weekend_sales_ratio.toFixed(1)}%</p>
          </div>
        </div>
      </div>
    </div>
