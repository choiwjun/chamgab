...
async function distinctSigunguCount(admin: AdminClient, table: string): Promise<number> {
...
async function latestMonth(admin: AdminClient, table: string): Promise<string | null> {
  const { data, error } = await admin
    .from(table)
...
async function fetchLatestDistrictProfiles(admin: AdminClient): Promise<Map<string, AnyRow>> {
...
function resolveIndustryCluster(industryCode: string): string {
  ...
  if (industryCode.startsWith('D') || industryCode.startsWith('R') || industryCode.startsWith('N')) {
    return 'retail'
  }
  if (industryCode === 'S01') return 'academy'
  if (industryCode === 'S02') return 'fitness'
  if (industryCode.startsWith('I') || industryCode.startsWith('S')) return 'service'
  return 'other'
}
...
function estimateRawProbability(args: {
  ...
  const storeComp =
    args.storeCount < 10 ? 3 : args.storeCount < 30 ? 5 : args.storeCount <= 300 ? 8 : 6
  return round2(clamp(10 + survivalComp + salesComp + growthComp + franchiseComp + storeComp, 0, 100))
}
...
function calcRuleConfidence(args: {
  ...
  return round2(clamp(score, 30, args.hasBiz && args.hasSales && args.hasStore ? 90 : 82))
}
...
export async function computeCommercialQualitySnapshot(): Promise<CommercialQualitySnapshotPayload> {
  ...
  const [
    ...
    fetchLatestByCombo(admin, 'sales_statistics', 'monthly_avg_sales,sales_growth_rate'),
    fetchLatestByCombo(admin, 'store_statistics', 'store_count,franchise_count'),
  ...
  for (const key of allKeys) {
    ...
    const finalProbability = round2(clamp(calibratedProbability + fit.adjustment, 0, 100))
    ...
    const maxFreshness = Math.max(bizFreshness ?? 0, salesFreshness ?? 0, storeFreshness ?? 0)
    if (maxFreshness > COMMERCIAL_THRESHOLDS.freshnessMonthsMax) staleComboCount += 1
    ...
    const calibrationPenalty = Math.min(18, Math.abs(rawProbability - calibratedProbability) * 0.8)
  ...
  const lowProbHighConfPct = comboCount > 0 ? round2((lowProbHighConfCount / comboCount) * 100) : 0
  const highBucketPct = comboCount > 0 ? round2((highBucketCount / comboCount) * 100) : 0
  ...
  const mojibakeDetectedCount = detectMojibakeCount(
    [...Object.values(INDUSTRY_NAMES), ...Object.values(FACTOR_NAME_MAP)].map((value) =>
      String(value || '')
    )
  )
...
export async function insertCommercialQualitySnapshot(snapshot: CommercialQualitySnapshotPayload) {
  const admin = createAdminClient()
  const { data, error } = await admin
...
export function evaluateCommercialSnapshotGate(snapshot: AnyRow | null): {
  ...
  const lowProbHighConfPct = toNumber(snapshot.low_prob_high_confidence_ratio_pct)
  ...
  const mojibakeDetectedCount = toNumber((snapshot.details as AnyRow | null)?.mojibake_detected_count)
  ...
  const checks: CommercialGateCheck[] = [
    ...
    },
    {
      ...
      pass: coverageMin != null && coverageMin >= COMMERCIAL_THRESHOLDS.sigunguCoverageMin,
    ...
    },
    {
      ...
      pass: freshnessMonthsMax != null && freshnessMonthsMax <= COMMERCIAL_THRESHOLDS.freshnessMonthsMax,
    ...
    },
    {
      ...
      pass: snapshotAgeHours != null && snapshotAgeHours <= COMMERCIAL_THRESHOLDS.snapshotAgeHoursMax,
  ...
  return {
    ...
    metrics: {
      ...
      low_prob_high_confidence_count: toNumber(snapshot.low_prob_high_confidence_count),
      low_prob_high_confidence_ratio_pct: lowProbHighConfPct,
