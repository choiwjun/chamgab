...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [
      ...
      countExact(admin.from('chamgab_analyses').select('id', { count: 'exact', head: true })),
      countExact(admin.from('price_factors').select('id', { count: 'exact', head: true })),
      ...
      countExact(
        admin.from('schools').select('school_id', { count: 'exact', head: true }).eq('is_active', true)
      ),
      countExact(
        admin
    ...
    const avgFactorCount =
      totalAnalyses > 0 ? Number((totalFactors / totalAnalyses).toFixed(2)) : null
    ...
    const unmappableRows = toNumber(
      (autofixSummary?.backfill as Record<string, unknown> | undefined)?.unmappable_rows
    )
    ...
    const commercialChecks: GateCheck[] = commercialGate.checks.map((check) => ({
      key: check.key,
      label: check.label,
      value: check.value,
      target: check.target,
    ...
    const missingMetrics = [...apartmentChecks, ...commercialChecks, ...schoolChecks]
    ...
    return NextResponse.json({
      ...
      status: {
        ...
        overall: apartmentPass && commercialPass && schoolPass ? 'GO' : 'NO_GO',
      },
      apartment: {
      ...
      note:
        'Launch gate is metrics-only. Missing metrics are treated as FAIL until quality jobs publish them.',
    })
  } catch (error) {
    const message =
