...
function getGapBand(
  ...
  const absGapPct = Math.abs((chamgabPrice - latestTxPrice) / latestTxPrice) * 100
...
function buildQualityFlags(params: {
  ...
  else if (params.factorCount < params.factorTarget) flags.push('FACTOR_INCOMPLETE')
...
export async function buildChamgabQuality(
  ...
  const factorTarget = Math.max(1, Math.trunc(params.factorTarget ?? DEFAULT_FACTOR_TARGET))
