...
const COMMERCIAL_QUALITY_VERSION = process.env.COMMERCIAL_QUALITY_VERSION || 'commercial-quality-v1'
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          detail = String((parsed && (parsed.detail || parsed.error || parsed.message)) || '')
        } catch {
          detail = text
        }
...
function calcRuleBasedConfidence(args: {
  ...
  score += recencyBonus(args.bizRows) + recencyBonus(args.salesRows) + recencyBonus(args.storeRows)

  const fullCoverage = args.hasBiz && args.hasSalesRows && args.hasStoreRows
  return round1(clamp(score, 30, fullCoverage ? 90 : 82))
}

function calcCalibrationPenalty(rawProbability: number, calibratedProbability: number): number {
  return round1(Math.min(18, Math.abs(rawProbability - calibratedProbability) * 0.8))
}
...
function buildQualityFlags(args: {
  ...
  hasSalesRows: boolean
  hasStoreRows: boolean
  staleMonths: { biz: number | null; sales: number | null; store: number | null }
  confidence: number
  ...
  if ((args.staleMonths.biz ?? 0) > 3 || (args.staleMonths.sales ?? 0) > 3 || (args.staleMonths.store ?? 0) > 3) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (storesLatest.length > 0) {
      const totalStores = storesLatest.reduce((sum, row) => sum + (Number(row.store_count) || 0), 0)
      const totalFranchise = storesLatest.reduce(
        (sum, row) => sum + (Number(row.franchise_count) || 0),
        0
      )
      storeCount = totalStores > 0 ? totalStores : num(storesLatest[0].store_count) || null
      franchiseRatio = totalStores > 0 ? totalFranchise / totalStores : null
    }

    const features = {
      survival_rate: numOrNull(params.get('survival_rate')) ?? survivalRate ?? 50.0,
      monthly_avg_sales:
        numOrNull(params.get('monthly_avg_sales')) ?? monthlyAvgSales ?? 20_000_000,
      sales_growth_rate: numOrNull(params.get('sales_growth_rate')) ?? salesGrowthRate ?? 0.0,
      store_count: numOrNull(params.get('store_count')) ?? storeCount ?? 80,
      franchise_ratio: numOrNull(params.get('franchise_ratio')) ?? franchiseRatio ?? 0.15,
    ...
    const baseYearMonth = {
      business: bizLatest.length > 0 ? String(bizLatest[0].base_year_month || '') : null,
      sales: salesLatest.length > 0 ? String(salesLatest[0].base_year_month || '') : null,
      store: storesLatest.length > 0 ? String(storesLatest[0].base_year_month || '') : null,
    }
    const staleMonths = {
    ...
    if (mlCall.ok) {
      const mlProbability = round1(clamp(Number(mlCall.data.success_probability) || 0, 0, 100))
      const hasRaw =
      ...
      const rawProbability = hasRaw
        ...
        : mlProbability
      const calibratedProbability = hasRaw ? mlProbability : compressMlProbability(mlProbability)
      ...
      const calibrationPenalty = calcCalibrationPenalty(rawProbability, calibratedProbability)
      ...
      if (industryFit.probability_adjustment !== 0) {
        factors.push({
          ...
          direction: industryFit.probability_adjustment > 0 ? 'positive' : 'negative',
        })
      }
    ...
    const calibratedProbability = compressMlProbability(fallback.success_probability)
    const finalProbability = round1(
      clamp(calibratedProbability + industryFit.probability_adjustment, 0, 100)
    )
    const confidence = round1(
      clamp(ruleConfidence - industryFit.policy_penalty, 30, hasFullCoverage ? 90 : 82)
    )
